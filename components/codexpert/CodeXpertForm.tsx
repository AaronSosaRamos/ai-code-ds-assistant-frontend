...
const CodeXpertForm = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen transition-all duration-500 ease-in-out bg-gray-100 dark:bg-gray-900">
      <div className="w-full max-w-xl p-8 shadow-lg rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white transition-all duration-300 ease-in-out hover:shadow-2xl animate-fade-in">
        <h2 className="text-2xl font-extrabold mb-6 flex items-center space-x-2">
          <FaCheckCircle className="text-indigo-500 animate-pulse" /> <span>Code Optimization Form - CodeXpert</span>
        </h2>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            <label className="block text-sm font-medium flex items-center space-x-2">
              <FaCode className="text-indigo-500" /> <span>Code 📝</span>
            </label>
            <textarea
              {...register("code")}
              rows={4}
              className={`mt-2 block w-full border rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm p-3 transition-transform duration-300 ease-in-out ${errors.code ? "border-red-500 animate-shake" : "border-gray-300 dark:border-gray-700"
                } bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white`}
              placeholder="Enter your code here"
            />
            {errors.code && (
              <p className="text-red-500 text-sm flex items-center mt-2">
                <FaTimesCircle className="mr-2" /> {errors.code.message}
              </p>
            )}
          </div>
          ...
          <div>
            <label className="block text-sm font-medium flex items-center space-x-2">
              <FaCheckCircle className="text-indigo-500" /> <span>Programming Language 💻</span>
            </label>
            <input
              {...register("programming_language")}
              className={`mt-2 block w-full border rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm p-3 transition-transform duration-300 ease-in-out ${errors.programming_language ? "border-red-500 animate-shake" : "border-gray-300 dark:border-gray-700"
                } bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white`}
              placeholder="e.g., Python, Java"
            />
            {errors.programming_language && (
              <p className="text-red-500 text-sm flex items-center mt-2">
                <FaTimesCircle className="mr-2" /> {errors.programming_language.message}
              </p>
            )}
          </div>
          ...
          <div>
            <label className="block text-sm font-medium flex items-center space-x-2">
              <FaRobot className="text-indigo-500" /> <span>Is this AI-related code? 🤖</span>
            </label>
          ...
          <div>
            <label className="block text-sm font-medium flex items-center space-x-2">
              <FaCheckCircle className="text-indigo-500" /> <span>Context 🖋️</span>
            </label>
            <textarea
              {...register("context")}
              rows={3}
              className={`mt-2 block w-full border rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm p-3 transition-transform duration-300 ease-in-out ${errors.context ? "border-red-500 animate-shake" : "border-gray-300 dark:border-gray-700"
                } bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white`}
              placeholder="Describe the problem of your code"
            />
            {errors.context && (
              <p className="text-red-500 text-sm flex items-center mt-2">
                <FaTimesCircle className="mr-2" /> {errors.context.message}
              </p>
            )}
          </div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className={`px-6 py-3 font-medium text-white bg-indigo-600 rounded-lg shadow-md hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-transform duration-300 ${loading ? "opacity-50 cursor-not-allowed" : "hover:scale-105"
